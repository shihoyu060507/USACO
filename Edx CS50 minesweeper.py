...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0]-1,cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width and (i,j) not in self.mines and (i,j) not in self.safes: notknowncells.append((i,j))

        new_sentence = Sentence(notknowncells, count-cntMines) 
    ...
    def make_safe_move(self):
        ...
        move_steps = self.safes - self.moves_made
        if move_steps:
            return random.choice(list(move_steps))
